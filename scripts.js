  ...
  */
  ...
  const deleteRowsLista_ = (id_lista) => {
    try {
      var tableHeaderRowCount = 1;
      var table = document.getElementById(id_lista);
      var rowCount = table.rows.length;
  ...
  */
  ...
  const criarBtnVisualizar = (parent, id) => {
    ...
    imgsrc.onclick = function () {
      ...
      let inputDataInicio = document.getElementById("dataInicio").value;
      let inputDataFim = document.getElementById("dataFim").value;
      imprimi_dados('imp.html', {dtincio: inputDataInicio, dtfim: inputDataFim, id_pro: id_proc});
    }
  }
  ...
  */
  const somaTotalReceita = (valor) => {
    ...
    document.getElementById("totalServiço").innerHTML = formatMoedaLista(resultRec);
  ...
  */
  ...
  /*
    ...
    --------------------------------------------------------------------------------------
  */
  const altera_abs = (aba) => { 
  
    if(aba == 1){
      document.getElementById("btn_1").className = "button_1";
      document.getElementById("btn_2").className = "button_2";
      document.getElementById("btn_3").className = "button_2";
      document.getElementById("btn_4").className = "button_2";
      document.getElementById("aba_1").className = "mostra_aba";
      document.getElementById("aba_2").className = "esconde_aba";  
      document.getElementById("aba_3").className = "esconde_aba"; 
      document.getElementById("aba_4").className = "esconde_aba";
    }
    
    if(aba == 2){
      document.getElementById("btn_1").className = "button_2";
      document.getElementById("btn_2").className = "button_1";
      document.getElementById("btn_3").className = "button_2";
      document.getElementById("btn_4").className = "button_2";
      document.getElementById("aba_1").className = "esconde_aba";
      document.getElementById("aba_2").className = "mostra_aba"; 
      document.getElementById("aba_3").className = "esconde_aba"; 
      document.getElementById("aba_4").className = "esconde_aba";
    }
    ...
    if(aba == 3){
      document.getElementById("btn_1").className = "button_2";
      document.getElementById("btn_2").className = "button_2";
      document.getElementById("btn_3").className = "button_1";
      document.getElementById("btn_4").className = "button_2";
      document.getElementById("aba_1").className = "esconde_aba";
      document.getElementById("aba_2").className = "esconde_aba";
      document.getElementById("aba_3").className = "mostra_aba";
      document.getElementById("aba_4").className = "esconde_aba";
    }
    ...
    if(aba == 4){
      ...
      document.getElementById("btn_2").className = "button_2";
      document.getElementById("btn_3").className = "button_2";
      document.getElementById("btn_4").className = "button_1";
      document.getElementById("aba_1").className = "esconde_aba";
      document.getElementById("aba_2").className = "esconde_aba";
      document.getElementById("aba_3").className = "esconde_aba";
      document.getElementById("aba_4").className = "mostra_aba";
    }
  }
  ...
  */
  ...
  /*
    ...
    --------------------------------------------------------------------------------------
    --------------------------------------------------------------------------------------
    chama a função, para carregar os dados da impressão
    --------------------------------------------------------------------------------------
  */
  
  const imprimi_dados = (path, params, method) => {
    method = method || "post"; // Set method to get by default if not specified.
    var url = path + "?" + Object.keys(params).map(key => `${encodeURIComponent(key)}=${encodeURIComponent(params[key])}`).join("&");
    window.open(url, "_blank");
  }
 
  /*
    --------------------------------------------------------------------------------------
    Função recupera os dados POST enviado, pelo index.html
    --------------------------------------------------------------------------------------
  */
  const recuperaDadosPOST = () => {
    var dados = [];
    var array;

    if (window.location.search) {
        var queryString = window.location.search.substring(1); 
        dados = (decodeURIComponent(queryString));
        array = dados.replace("&dtincio", "").replace("&dtfim","").replace("&id_pro","");
        array = array.split("=");
    }
    impPeriodoReceita(array[1],array[2],array[3]);
    impPeriodoDespesa(array[1],array[2],array[3]);
  }

  /*
    --------------------------------------------------------------------------------------
    Função carregar dados da receita de processado, para impressão
    --------------------------------------------------------------------------------------
  */
  const impPeriodoReceita = (DataInicio, DataFim, id_proc) => {
    let url = 'http://127.0.0.1:5000/receita_periodo?id_proc=' + id_proc + '&datainicio=' + DataInicio + '&datafim=' + DataFim;
    fetch(url, {
      method: 'get',
    })
    .then((response) => response.json())
    .then((data) => {
        data.receitas.forEach(_item => impLista(_item.id,_item.descricao, formatDataLista(_item.data),_item.valor,_item.status,'receita_', true))
    })
    .catch((error) => {
      console.error('Error:', error);
    });
  ...
  /*
    --------------------------------------------------------------------------------------
    Função carregar dados da despesa de processado, para impressão
    --------------------------------------------------------------------------------------
  */
  var periodo;
  const impPeriodoDespesa = (DataInicio, DataFim, id_proc) => {
    periodo = formatDataLista(DataInicio) +' HA '+ formatDataLista(DataFim);
    let url = 'http://127.0.0.1:5000/despesa_periodo?id_proc=' + id_proc + '&datainicio=' + DataInicio + '&datafim=' + DataFim;
    fetch(url, {
      method: 'get',
    })
    .then((response) => response.json())
    .then((data) => {
        data.despesas.forEach(_item => impLista(_item.id,_item.descricao, formatDataLista(_item.data),_item.valor,_item.status,'despesa_', true))
    })
    .catch((error) => {
      console.error('Error:', error);
    })
  ...
  /*
    --------------------------------------------------------------------------------------
    Função carregar a lista para impressão
    --------------------------------------------------------------------------------------
  */
  const impLista = (id, descricao, data, valor, status, processar) => {

    var item = [id, descricao, data, formatMoedaLista(valor)];
    var table = document.getElementById(processar);
    var row = table.insertRow();
    ...
    if(status == 1){
      if(processar == 'receita_'){
        somaTotalReceita_imp(valor, 'totalServiço_'); 
      }else{
        somaTotalDespesa_imp(valor, 'totalDespesa_'); 
      } 
    }
  }
  ...
  /*
    --------------------------------------------------------------------------------------
    Função soma total de recita
    --------------------------------------------------------------------------------------
  */
  const somaTotalReceita_imp = (valor, cptotal) => {
    resultRec_+=valor;
    if(resultRec_ < 0){resultRec_ = 0;}
    document.getElementById(cptotal).innerHTML = formatMoedaLista(resultRec_);
  }
 
  /*
    --------------------------------------------------------------------------------------
    Função soma total de despesa
    --------------------------------------------------------------------------------------
  */
  const somaTotalDespesa_imp = (valor, cptotal) => {
    resultDesp_+=valor;
    if(resultDesp_ < 0){resultDesp_ = 0;}
    document.getElementById(cptotal).innerHTML = formatMoedaLista(resultDesp_);
  }
  
  /*
    --------------------------------------------------------------------------------------
    Função carrega lista de resumo
    --------------------------------------------------------------------------------------
  */
  const impResumoMensal = () => {
    
    var item = [periodo, formatMoedaLista(resultRec_), formatMoedaLista(resultDesp_), formatMoedaLista((resultRec_-resultDesp_))];
    var table = document.getElementById('resumo_');
  ...
  */
  ...
  /*
    --------------------------------------------------------------------------------------
    Função Pesquisa de produtos no mercado livre
    --------------------------------------------------------------------------------------
  */
  const buscaPesquisa = () => {
    
    var query = $("#query").val();
    var limit = 10;  // Número padrão de resultados

    let url = "http://127.0.0.1:5000/pesquisar_pecas?pesquisa=" + query + "&limite=" + limit;
    fetch(url, {
      method: 'get',
    })
    .then((response) => response.json())
    .then((data) => { 
      ...
      var html = "<h2>Resultados da pesquisa:</h2><ul>";
      ...
      if (results.length === 0) {
          ...
          for (var i = 0; i < results.length; i++) {
              ...
              html += "Preço: $" + item.price + "<br>";
              html += "Localização: " + item.address.state_name + "<br><br></li>";
          }
      }
      html += "</ul>";
